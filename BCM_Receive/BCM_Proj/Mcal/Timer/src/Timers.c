...
volatile uint8 Timer_gBcm_Rx_Flag;
...
void Timers_timer0_Delay_ms(uint16 delay)
{
	...
	while(counter--)
	{
		while( (TIFR & TIFR_TOV0_gBcm_Rx_Flag_MASK) == FALSE);
		TIFR |= TIFR_TOV0_gBcm_Rx_Flag_MASK;
	}
	Timers_timer0_Stop();
}
...
void Timers_timer1_Delay_ms(uint16 delay)
{
	...
	while(counter--)
	{
		while( (TIFR & TIFR_TOV1_gBcm_Rx_Flag_MASK) == FALSE);
		TIFR |= TIFR_TOV1_gBcm_Rx_Flag_MASK;
	}
	Timers_timer1_Stop();
}
...
void Timers_timer2_Delay_ms(uint16 delay)
{
	...
	while(counter--)
	{
		while( (TIFR & TIFR_TOV2_gBcm_Rx_Flag_MASK) == FALSE);
		TIFR |= TIFR_TOV2_gBcm_Rx_Flag_MASK;
	}
	Timers_timer2_Stop();
}
